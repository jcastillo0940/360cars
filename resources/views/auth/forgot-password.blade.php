...
@section('title', 'Recuperar contraseña | Movikaa')
...
<section class="min-h-[calc(100vh-5rem)] bg-black px-4 py-10 sm:px-6 lg:px-8">
    <div class="mx-auto flex min-h-[calc(100vh-10rem)] max-w-2xl items-center justify-center">
        <div class="w-full max-w-xl rounded-[2rem] border border-white/10 bg-[#0b0b0f] p-6 shadow-[0_30px_120px_rgba(0,0,0,0.55)] sm:p-8 lg:p-10">
            ...
            <h1 class="mt-3 font-headline text-3xl font-extrabold tracking-tight text-white sm:text-4xl">Recupera tu contraseña</h1>
            <p class="mt-3 text-sm leading-7 text-slate-300 sm:text-base">Ingresa tu correo y te enviaremos un enlace para crear una nueva contraseña.</p>
            ...
            <form method="POST" action="{{ route('password.email') }}" class="mt-8 grid gap-5">
                ...
                <label class="grid gap-2">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required class="min-h-14 rounded-2xl border border-white/10 bg-[#12131a] px-4 text-white shadow-sm outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/15" placeholder="tu@correo.com" />
                </label>
                <button type="submit" class="inline-flex min-h-14 items-center justify-center rounded-2xl bg-secondary px-6 font-headline text-base font-extrabold text-white shadow-lg transition hover:bg-secondary-container">Enviar enlace</button>
            </form>

            <div class="mt-6 flex flex-wrap gap-3">
                <a href="{{ route('login') }}" class="inline-flex min-h-12 items-center justify-center rounded-2xl border border-white/10 bg-white/5 px-5 text-sm font-bold text-white transition hover:border-primary hover:bg-white/10">Volver a iniciar sesión</a>
