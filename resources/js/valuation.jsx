                ...
                <div className="space-y-4">
                    <div className={`rounded-2xl border p-5 shadow-sm ${isDark ? 'border-white/10 bg-white/5' : 'border-outline-variant/20 bg-white'}`}>
                        ...
                        <div className={`mt-4 space-y-3 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                            ...
                            <div className="flex items-center justify-between gap-3"><span>TransmisiÃ³n</span><strong>{result.snapshot.transmission}</strong></div>
                                ...
                                <section className={`rounded-[28px] border p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] sm:p-8 ${isDark ? 'border-white/10 bg-[#0b1118]' : 'border-outline-variant/25 bg-white'}`}>
                                    ...
                                    <form action={submitUrl} method="POST" className="grid gap-6">
                                        ...
                                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                            ...
                                            <label className="flex flex-col gap-2"><span className="text-xs font-bold uppercase tracking-[0.2em] text-primary">TransmisiÃ³n</span><select name="transmission" required className="rounded-2xl border border-outline-variant/30 bg-white px-4 py-4 text-sm font-semibold text-slate-800 shadow-sm outline-none transition focus:border-primary"><option value="">Selecciona</option>{vehicleConfig.transmissions.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
                                            <label className="flex flex-col gap-2"><span className="text-xs font-bold uppercase tracking-[0.2em] text-primary">TracciÃ³n</span><select name="drivetrain" className="rounded-2xl border border-outline-variant/30 bg-white px-4 py-4 text-sm font-semibold text-slate-800 shadow-sm outline-none transition focus:border-primary"><option value="">No especificar</option>{vehicleConfig.drivetrains.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
                                        </div>
                                        ...
                                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                            ...
                                            <label className="flex flex-col gap-2"><span className="text-xs font-bold uppercase tracking-[0.2em] text-primary">TamaÃ±o del motor</span><input type="number" name="engine_size" min="0" max="10" step="0.1" placeholder="Ej. 2.5" className="rounded-2xl border border-outline-variant/30 bg-white px-4 py-4 text-sm font-semibold text-slate-800 shadow-sm outline-none transition focus:border-primary" /></label>
                                        ...
                                        <label className="flex flex-col gap-2 rounded-2xl border border-outline-variant/25 bg-slate-50 p-5">
                                            ...
                                            <small className="text-sm text-slate-500">El sistema devolverÃ¡ el valor en CRC como precio principal y una referencia pequeÃ±a en USD.</small>
                                        </label>
                                        ...
                                        <div className="flex flex-col gap-4 rounded-2xl bg-secondary px-5 py-5 text-white sm:flex-row sm:items-center sm:justify-between">
                                            <div>
                                                <span className="text-xs font-bold uppercase tracking-[0.2em] text-white/70">Resultado</span>
                                                <p className="mt-2 text-sm text-white/90">RecibirÃ¡s un rango de mercado, una lectura de depreciaciÃ³n y un enlace para compartir la evaluaciÃ³n.</p>
                                            </div>
                                            <button type="submit" className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white transition hover:bg-black"><Icon name="query_stats" className="text-[18px]" />Calcular valuaciÃ³n</button>
                                        </div>
                                    </form>
                                </section>
                            )}
