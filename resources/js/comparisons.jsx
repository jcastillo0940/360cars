...
function ComparisonCard({ vehicle, onRemove }) {
    return (
        <article className="overflow-hidden rounded-[2rem] border border-outline-variant/20 bg-white shadow-xl">
            ...
            <div className="p-6">
                <div className="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <a href={vehicle.url} className="mt-2 block font-headline text-2xl font-extrabold tracking-tight text-slate-950">{vehicle.title}</a>
                    </div>
                    <button type="button" onClick={() => onRemove(vehicle.id)} className="rounded-full bg-slate-100 p-3 text-slate-500 transition hover:bg-red-50 hover:text-red-600" aria-label="Quitar del comparador">
                        <Icon name="close" className="text-[18px]" />
                    </button>
                </div>

                <div className="mt-5 flex flex-wrap gap-2">
                    {vehicle.performance_badge ? <span className="rounded-full bg-primary-fixed px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-primary">{vehicle.performance_badge}</span> : null}
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-slate-600">{vehicle.view_count} vistas</span>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-slate-600">{vehicle.lead_count} contactos</span>
                </div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-2xl border border-outline-variant/20 bg-slate-50 p-4">
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Año</p>
                        <strong className="mt-2 block text-slate-900">{vehicle.year || 'N/D'}</strong>
                    </div>
                    <div className="rounded-2xl border border-outline-variant/20 bg-slate-50 p-4">
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Kilometraje</p>
                        <strong className="mt-2 block text-slate-900">{vehicle.mileage ? `${vehicle.mileage} ${vehicle.mileage_unit}` : 'N/D'}</strong>
                    </div>
                    <div className="rounded-2xl border border-outline-variant/20 bg-slate-50 p-4">
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Combustible</p>
                        <strong className="mt-2 block text-slate-900">{vehicle.fuel_type || 'N/D'}</strong>
                    </div>
                    <div className="rounded-2xl border border-outline-variant/20 bg-slate-50 p-4">
                        <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Transmisión</p>
                        <strong className="mt-2 block text-slate-900">{vehicle.transmission || 'N/D'}</strong>
                    </div>
                </div>

                <div className="mt-6 flex flex-wrap gap-3">
                    <a href={vehicle.url} className="inline-flex items-center justify-center rounded-2xl border border-outline-variant/20 px-4 py-3 font-bold text-slate-700 transition hover:border-primary hover:text-primary">Ver ficha</a>
                    {vehicle.contact_url ? (
                    ...
                    <div className="mx-auto max-w-screen-2xl px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="mt-4 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                            ...
                            <div className="flex flex-wrap gap-3">
                                <a href={catalogUrl} className="inline-flex items-center justify-center rounded-2xl border border-outline-variant/25 bg-white px-5 py-4 font-bold text-slate-700 transition hover:border-primary hover:text-primary">Agregar otro auto</a>
                                {ids.length ? (
                        ...
                        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div className="rounded-2xl border border-outline-variant/20 bg-white/85 p-5 shadow-sm">
                                <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Autos comparados</p>
                                <strong className="mt-3 block font-headline text-4xl font-extrabold text-primary">{comparisonVehicles.length}</strong>
                            </div>
                            <div className="rounded-2xl border border-outline-variant/20 bg-white/85 p-5 shadow-sm">
                                <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Rango de precio</p>
                                <strong className="mt-3 block font-headline text-2xl font-extrabold text-slate-950">{priceSummary}</strong>
                            </div>
                            <div className="rounded-2xl border border-outline-variant/20 bg-white/85 p-5 shadow-sm">
                                <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Estado</p>
                                <strong className="mt-3 block font-headline text-2xl font-extrabold text-slate-950">{comparisonVehicles.length >= 2 ? 'Listo para comparar' : 'Agrega otro auto'}</strong>
                            </div>
                        </div>
                    </div>
                </section>
                    ...
                    {comparisonVehicles.length ? (
                        <>
                            ...
                            <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                                {comparisonVehicles.map((vehicle) => <ComparisonCard key={vehicle.id} vehicle={vehicle} onRemove={removeVehicle} />)}
                            </div>

                            {comparisonRecommendation ? (
                                <div className="mt-10 grid grid-cols-1 gap-6 xl:grid-cols-[0.9fr_1.1fr]">
                                    <article className="rounded-[2rem] border border-outline-variant/20 bg-white p-6 shadow-xl sm:p-8">
                                        <p className="text-xs font-bold uppercase tracking-[0.24em] text-secondary">Lectura rápida</p>
                                        <h2 className="mt-3 font-headline text-3xl font-extrabold tracking-tight text-slate-950">La opción más equilibrada</h2>
                                    ...
                                    <article className="rounded-[2rem] border border-outline-variant/20 bg-white p-6 shadow-xl sm:p-8">
                                        ...
                                        <h2 className="mt-3 font-headline text-3xl font-extrabold tracking-tight text-slate-950">Cómo quedaron ordenados</h2>
                                        <div className="mt-6 space-y-4">
                                            {comparisonRecommendation.ranking.map((item, index) => (
                                                <div key={item.vehicle_id} className="rounded-2xl border border-outline-variant/20 bg-slate-50 p-5">
                                                    <div className="flex flex-wrap items-center justify-between gap-3">
                                                        <div>
                                                            <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">#{index + 1}</p>
                                                            <a href={item.url} className="mt-2 block font-headline text-2xl font-extrabold text-slate-950">{item.title}</a>
                                                            <p className="mt-2 text-sm text-slate-500">{item.headline}</p>
                                                        </div>
                                                        <span className="rounded-full bg-primary-fixed px-4 py-2 text-sm font-black text-primary">{item.score}/100</span>
                                                    </div>
                                                    <p className="mt-4 text-sm leading-7 text-slate-600">{item.reasons.join('. ')}.</p>
                                                </div>
                                            ))}
                                        </div>
                                    </article>
                        ...
                        <div className="rounded-[2rem] border border-outline-variant/20 bg-white p-10 text-center shadow-xl">
                            ...
                            <h2 className="mt-4 font-headline text-3xl font-extrabold tracking-tight text-slate-950">Tu comparador está vacío.</h2>
                            <p className="mt-3 text-base leading-8 text-slate-600">Primero elige un auto en el inventario, luego agrega otro para compararlos aquí con el mismo look del frontend.</p>
                ...
                <section className="mx-auto max-w-screen-2xl px-4 pb-16 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                        {suggestedVehicles.map((vehicle) => (
                            <article key={vehicle.id} className="overflow-hidden rounded-2xl border border-outline-variant/20 bg-white shadow-lg">
